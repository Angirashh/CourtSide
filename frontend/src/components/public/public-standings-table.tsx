      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-[11px] uppercase tracking-wide text-navy-400">
            ...
            <th className="hidden px-2 py-2 text-center font-semibold sm:table-cell">Diff</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.player_id} className={cn('border-t border-cream-200', row.rank <= 2 && 'bg-ember-100/40')}>
              <td className="px-4 py-2.5 font-bold text-navy-400">{row.rank}</td>
              <td className="px-2 py-2.5 font-semibold text-navy-900">{row.player_name}</td>
